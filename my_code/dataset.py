...
from data.femnist_dataset import FEMNIST, femnist_replace    
...
class TinyImageNet:
    ...
    def data_loaders(
        ...
        print(
            f"Traing loader: {len(train_loader.train_set)} images, Test loader: {len(test_loader.train_set)} images"
        )
        return train_loader, val_loader, test_loader


def femnist_dataloaders(
    batch_size=128,
    num_workers=2,
    seed: int = 1,
    shuffle=True,
    ...
    train_set = FEMNIST(train=True, transform=train_transform)
    # subset_size = 33780
    # indices = rng.choice(len(train_set), size=subset_size, replace=False)
    # train_set.data = np.array([train_set.data[i] for i in indices])
    # train_set.labels = np.array([train_set.labels[i] for i in indices])
    train_set.data = np.array(train_set.data)
    train_set.labels = np.array(train_set.labels)
    valid_set = copy.deepcopy(train_set)
    ...
    # for i in range(max(train_set.labels) + 1):
    #     class_idx = np.where(train_set.labels == i)[0]
    #     nb_samples = int(0.1 * len(class_idx))

    #     valid_idx = rng.choice(class_idx, nb_samples, replace=False)
    #     class_idx = list(set(class_idx) - set(valid_idx))
    #     test_idx = rng.choice(class_idx, nb_samples, replace=False)
    #     train_idx = np.array(list(set(class_idx) - set(test_idx)))
    #     all_train_idx = np.concatenate([all_train_idx,train_idx]).astype(int)
    #     all_test_idx = np.concatenate([all_test_idx,test_idx]).astype(int)
    #     all_valid_idx = np.concatenate([all_valid_idx,valid_idx]).astype(int)

    #     for user_id, samples in train_set_copy.dic_users.items():
    #         if [idx in valid_idx for idx in samples].count(True) > 0:
    #             if user_id not in valid_set.dic_users:
    #                 valid_set.dic_users[user_id] = []
    #             valid_set.dic_users[user_id] += [np.where(idx==all_valid_idx)[0][0] for idx in samples if idx in valid_idx]

    #         # Ajouter les indices de test

    #         if [idx in test_idx for idx in samples].count(True) > 0:
    #             if user_id not in test_set.dic_users:
    #                 test_set.dic_users[user_id] = []
    #             test_set.dic_users[user_id] += [np.where(idx==all_test_idx)[0][0] for idx in samples if idx in test_idx]

    #         # Ajouter les indices d'entraînement

    #         if [idx in train_idx for idx in samples].count(True) > 0:
    #             if user_id not in train_set.dic_users:
    #                 train_set.dic_users[user_id] = []
    #             train_set.dic_users[user_id] += [np.where(idx==all_train_idx)[0][0] for idx in samples if idx in train_idx]
    # print('Validation, Test and Train sets created')

    nb_samples = int(0.1 * len(train_set))
    class_idx = np.arange(len(train_set))
    valid_idx = rng.choice(class_idx, nb_samples, replace=False)
    class_idx = list(set(class_idx) - set(valid_idx))
    test_idx = rng.choice(class_idx, nb_samples, replace=False)
    train_idx = np.array(list(set(class_idx) - set(test_idx)))
    ...
    for user_id, samples in train_set_copy.dic_users.items():
        ...
        if [idx in train_idx for idx in samples].count(True) > 0:
            ...
            train_set.dic_users[user_id] += [np.where(idx==all_train_idx)[0][0] for idx in samples if idx in train_idx]
    print('Validation, Test and Train sets created')
    #Mise à jour des données et labels pour chaque ensemble
    valid_set.data = train_set_copy.data[all_valid_idx]
    valid_set.labels = train_set_copy.labels[all_valid_idx]
    test_set.data = train_set_copy.data[all_test_idx]
    test_set.labels = train_set_copy.labels[all_test_idx]

    train_set.data = train_set_copy.data[all_train_idx]
    train_set.labels = train_set_copy.labels[all_train_idx]
    selected_indices_test_svm,selected_indices_forget = femnist_replace(train_set, seed,percentage_limit_1=0.1,percentage_limit_2=0.02)
    svm_dataset_test = copy.deepcopy(train_set)
    svm_dataset_test.data = svm_dataset_test.data[selected_indices_test_svm]
    svm_dataset_test.labels = svm_dataset_test.labels[selected_indices_test_svm]
    if class_to_replace is not None:
        train_set.labels[selected_indices_forget] = -train_set.labels[selected_indices_forget] - 1

    mask = np.ones(len(train_set), dtype=bool)
    mask[selected_indices_test_svm] = False
    ...
    return train_loader, val_loader, test_loader, svm_dataset_test


def replace_indexes(
    train_set: torch.utils.data.Dataset, indexes, seed=0, only_mark: bool = False
):
    if not only_mark:
        rng = np.random.RandomState(seed)
        new_indexes = rng.choice(
            list(set(range(len(train_set))) - set(indexes)), size=len(indexes)
        )
        train_set.data[indexes] = train_set.data[new_indexes]
        try:
            train_set.targets[indexes] = train_set.targets[new_indexes]
        except:
            train_set.labels[indexes] = train_set.labels[new_indexes]
        else:
            train_set._labels[indexes] = train_set._labels[new_indexes]
    else:
        # Notice the -1 to make class 0 work
        try:
            train_set.targets[indexes] = -train_set.targets[indexes] - 1
        except:
            try:
                train_set.labels[indexes] = -train_set.labels[indexes] - 1
            except:
                train_set._labels[indexes] = -train_set._labels[indexes] - 1
...
def replace_class(
    ...
    if class_to_replace == -1:
        try:
            indexes = np.flatnonzero(np.ones_like(train_set.targets))
        except:
            try:
                indexes = np.flatnonzero(np.ones_like(train_set.labels))
            except:
                indexes = np.flatnonzero(np.ones_like(train_set._labels))
    else:
        try:
            indexes = np.flatnonzero(np.array(train_set.targets) == class_to_replace)
        except:
            try:
                indexes = np.flatnonzero(np.array(train_set.labels) == class_to_replace)
            except:
                indexes = np.flatnonzero(np.array(train_set._labels) == class_to_replace)

    if num_indexes_to_replace is not None:
        assert num_indexes_to_replace <= len(
            indexes
        ), f"Want to replace {num_indexes_to_replace} indexes but only {len(indexes)} samples in train_set"
        rng = np.random.RandomState(seed)
